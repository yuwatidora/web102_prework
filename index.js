...
const totalContrubutions = GAMES_JSON.reduce((acc,card) =>{
    return acc + card.backers;
},0)

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `
    <small>${totalContrubutions.toLocaleString('en-US')}</small>
`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");

const totalRaise = GAMES_JSON.reduce((acc, card) =>{
    return acc + card.pledged;
},0)

// set inner HTML using template literal
raisedCard.innerHTML = `
    <small>$${totalRaise.toLocaleString('en-US')}</small>
`;
...
const unFunded = listOfUnfunded.reduce((acc)=>{
    return acc + 1;
},0)
...
const numFunded = listOfFunded.reduce((acc)=>{
    return acc +1;
},0)
...
let[mostPledged, secondMostpledged, ...others] = sortedGames;
// create a new element to hold the name of the top pledge game, then append it to the correct element
const topGame = document.createElement("div");
topGame.innerHTML = `${mostPledged.name}`;
firstGameContainer.append(topGame);
// do the same for the runner up item
const secondTopGame = document.createElement("div");
secondTopGame.innerHTML = `${secondMostpledged.name}`;
